...
module.exports = {
  ...
  async handler(argv) {
    ...
    const { isCurrent, dirName, projectType: pType, repository } = answers;
    ...
    if (!err) {
      ...
      if (initAnswer.isInit) {
        ...
        const done = await sh.exec.npmrun('i -s').catch(e => {
          spinner.fail(`${chalk.red(`安装依赖等初始化操作失败，需手动执行 ${chalk.yellow.bold('yarn i')} 完成`)}`);
        });
        if (done) {
          spinner.succeed(
            `${chalk.green(
              `${answers.name} ${pType} 已初始化完成，${
                !isCurrent ? chalk.yellow.bold(` cd ${dirName} 进入项目，`) : ''
              }执行 ${chalk.yellow.bold(' yarn dev ')} 进入开发`
            )}\n`
          );
        }
      }
